...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->lang->load('ion_auth', $this->language->get());
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where($this->config->item('identity', 'ion_auth'), $username)
	...
	{
        $username = $email; // We are going to use email as username.
	 ...
	 * @author Mathew
	 **/
	public function login($identity, $password, $remember=FALSE, $nopasscheck = FALSE)
	{
		$this->trigger_events('pre_login');

		if ((empty($identity) || empty($password)) && !$nopasscheck)
		{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE || $nopasscheck === TRUE)
			{
				$session_data = array(
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists('username', $data) || array_key_exists('password', $data) || array_key_exists('email', $data))
		{
			if (array_key_exists('password', $data))
			{
				...
				{
					$data['password'] = $this->hash_password($data['password'], false);
				}
				else
				{
